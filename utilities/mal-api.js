 ...
 */
 ...
 export const requestNRandomAnime = async (number = 1, callback = null) => {
  ...
  for (let i = 0; i < number; ++i) {
    ...
    } while (itemsData.some((itemData) => animeData["mal_id"] === itemData["mal_id"]));
...
export const searchAnime = async (title, limit = 20) => {
  ...
  const filteredList = [];
  list.forEach((anime) => {
    if (anime["type"] === "Music" || anime.images.jpg["large_image_url"] === "https://cdn.myanimelist.net/img/sp/icon/apple-touch-icon-256.png" || isNSFW(anime) || !allowedTypes.includes(anime.type)) {
      return;
    }
