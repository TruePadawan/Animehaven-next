...
export async function getReviewUpvoteList(
  supabase: SupabaseClient<Database>,
  reviewId: Tables<"item_reviews">["id"],
) {
  const { data, error } = await supabase
    .from("item_reviews")
    .select("upvoted_by")
...
export async function reviewExist(
  supabase: SupabaseClient<Database>,
  reviewId: Tables<"item_reviews">["id"],
) {
  const { count } = await supabase
...
export async function toggleUpvoteForReview(
  supabase: SupabaseClient<Database>,
  reviewId: Tables<"item_reviews">["id"],
  profileId: Tables<"profiles">["id"],
) {
...
export async function toggleUpvoteForComment(
  supabase: SupabaseClient<Database>,
  commentId: Tables<"comments">["id"],
  profileId: Tables<"profiles">["id"],
) {
...
export async function getUserItemRecommendations(
  supabase: SupabaseClient<Database>,
  profileId: Tables<"profiles">["id"],
) {
  return supabase
...
export async function getUserItemReviews(
  supabase: SupabaseClient<Database>,
  profileId: Tables<"profiles">["id"],
) {
  return supabase
...
export async function getDiscussionsByTags(
  supabase: SupabaseClient<Database>,
  tags: string[],
  profileId: Tables<"profiles">["id"] | null = null,
) {
  if (profileId === null) {
...
export async function getDiscussionByID(
  supabase: SupabaseClient<Database>,
  discussionId: Tables<"discussions">["id"],
) {
  if (!discussionId) {
...
export async function getDiscussionByAccountName(
  supabase: SupabaseClient<Database>,
  accountName: string,
) {
  if (!accountName) {
  ...
  if (id === undefined) {
    throw new Error(`No account with account name ${accountName}`);
  }
  const { data, error } = await supabase
    .from("discussions")
    .select("*")
...
export async function getListByID(
  supabase: SupabaseClient<Database>,
  animelistId: Tables<"anime_lists">["id"],
) {
  if (!animelistId) {
...
export async function setRecentItem(
  supabase: SupabaseClient<Database>,
  type: string,
  profileId: string,
  item: any,
  ...
  if (!itemAlreadyExists) {
    ...
    try {
      ...
      throw new Error(
        `Failed to update recent items - ${getErrorMessage(error)}`,
      );
    }
  }
}
