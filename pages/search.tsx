...
export default function Search() {
    ...
    const [userSearchResult, setUserSearchResult] = useState<Tables<"profiles">[]>([])
    ...
    const searchFunc = useCallback(async (searchText: string, searchCategory: string) => {
        setLoading(true);
        if (searchCategory === "ANIME") {
            try {
                ...
                triggerAlert("Search failed!", {severity: "error", error: error as PostgrestError});
            }
        } else if (searchCategory === "USER") {
            const result = await supabase.rpc("search_user", {phrase: searchText});
        ...
        <Fragment>
            ...
            <Box
                ...
                <SearchBar
                    ...
                    searchCategory={typeof searchCategory === "string" ? searchCategory : "Anime"}
                />
